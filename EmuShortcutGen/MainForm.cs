...
public partial class MainForm : Form
{
    ...
    {
        if (
            string.IsNullOrWhiteSpace(txtGamesFolder.Text) || !Directory.Exists(txtGamesFolder.Text)
        )
        {
            MessageBox.Show(
                "Please select a valid games folder.",
                "Error",
    ...
    private async void GenerateShortcuts()
    {
        try
        {
            ...
            var gameFiles = Directory
                .GetFiles(gamesFolder, "*.*", SearchOption.TopDirectoryOnly)
                .Where(file =>
            ...
            foreach (var gameFile in gameFiles)
            {
                ...
                lblStatus.Text =
                    $"Processing {gameName}... ({shortcutsCreated + 1}/{gameFiles.Length})";
                Application.DoEvents();

                var searchQuery = txtSearchQuery.Text;
                var iconPath = await GetGameIconAsync(gameName, searchQuery);
                var arguments = string.IsNullOrWhiteSpace(txtArguments.Text) 
                    ? $"\"{gameFile}\"" 
                    : $"{txtArguments.Text} \"{gameFile}\"";
                CreateShortcut(
                    shortcutPath,
                    emulatorPath,
                    arguments,
                    Path.GetDirectoryName(emulatorPath),
                    $"Launch {gameName} with emulator",
                    iconPath
                );
                shortcutsCreated++;
            }
            ...
            MessageBox.Show(
                $"Generated {shortcutsCreated} shortcuts successfully!",
                "Success",
                MessageBoxButtons.OK,
    ...
    private async Task<string?> GetGameIconAsync(string gameName, string searchQuery)
    {
        try
        {
            ...
            var imageUrl = await GetFirstImageUrlAsync($"{gameName} {searchQuery}");
    ...
    private static async Task<string?> DownloadAndConvertIconAsync(string imageUrl, string iconPath)
    {
        try
        {
